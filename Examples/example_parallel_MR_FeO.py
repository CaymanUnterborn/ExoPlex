...
def run_planet(x, *args):
    ...
    compositional_params['wt_frac_FeO_wanted'] = x
    print(x)
    ...
    rho = 1/(pow(Planet['radius'][-1],3))
    rho_act = 1/(pow(Rad_planet*6371e3,3))

    #print(rho/6371e3)
    #print(rho_act/6371e3)
    out = 1 - (rho) / rho_act
    #print("out",out)
    return (out)
...
def calc_planet(mass, radius):
    ...
    try:
        FeO = root_scalar(run_planet,bracket=[min,max] ,args=(mass, radius),xtol=0.0001).root
    except:
        ...
        if abs(run_planet(FeO,*(mass,radius))) > tol:
...
if __name__ == "__main__":
    ...
    FeO = pool.starmap_async(calc_planet,vals).get()

    pool.close()

    for i in range(len(Mass_planet)):
        print(Mass_planet[i], Radius_planet[i], FeO[i])
    sys.exit()
    CMF = []
